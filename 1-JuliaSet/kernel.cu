/**
 * @file kernel.cu
 * @brief GPU-accelerated and CPU implementations of Julia Set fractal rendering.
 *
 * This file implements the generation and visualization of Julia Set fractals.
 * Points on the complex plane are iterated using the Julia function to determine color values.
 * The computation can run on CPU or GPU, and OpenGL is used for rendering the fractal in real-time.
 */
